...
function renderDuesTable() {
  ...
  state.apartments.forEach(apt => {
    ...
    // Row Summary (Güncel Borç: Current month included unpaid debt, future prepayments shown as positive)
...
function setupEventListeners() {
  ...
  const btnImportBackupClick = document.getElementById('btnImportBackupClick');
  if (btnImportBackupClick) {
    btnImportBackupClick.addEventListener('click', () => {
...
function handleExportBackup() {
  try {
    ...
    downloadAnchor.setAttribute("download", `cicek-apartmani-yedek-${new Date().toLocaleDateString('tr-TR')}.json`);
...
function handleImportBackup(event) {
  ...
  reader.onload = function(e) {
    try {
      ...
      if (!importedData.apartments || !Array.isArray(importedData.apartments)) {
        showToast("Hatalı dosya formatı! Yedek yüklenemedi.", "error");
        return;
      }
      ...
      initApp();
      closeModal('settingsModal');
      showToast("Yedek başarıyla yüklendi ve sistem güncellendi!", "success");
    } catch (err) {
      showToast("Dosya okunurken hata oluştu veya JSON geçersiz.", "error");
    }
  };
